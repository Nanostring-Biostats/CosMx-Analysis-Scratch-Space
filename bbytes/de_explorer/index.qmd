...
<!-- note: do not change the line below; it is replaced during deployment.  -->
...
if(length(new_zip_data) > 1){
  ...
  if(!isValid){
    ...
    # 4. Read the Header File using DuckDB
    tryCatch({
       con_peek <- dbConnect(duckdb())
       # Read the whole header row
...
// Reactively adds user info to the available_studies
{
...
viewof aesthetics_raw = Inputs.form({
  ...
  // Note: We can still customize width by creating the element first
...
// a bind of the contrast selection (placed in controls card)
viewof term_copy2 = {
  
  // 1. Safety check (same as the original)
  if (!available_terms || available_terms.length === 0) return md`_Loading..._`;

  // 2. Create the new visual input (Visual Duplicate)
...
<!-- ::::{.callout-note} -->
<!-- The variable 'df' is reserved for the data itself and is passed reactively  -->
<!-- during the execution. 'df' is appended to argsVolcano just before execution for -->
<!-- efficiency.  -->
<!-- :::: -->

# Console

You can use this for a generic R console. 
