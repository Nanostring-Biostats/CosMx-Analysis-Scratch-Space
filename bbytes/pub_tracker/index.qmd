...
<div class="container mx-auto px-6">
  <div class="mb-8">
    ...
    <p class="text-slate-500 text-lg mt-2">
      ...
      <strong class="text-slate-700">Holding shift while clicking any bubble</strong> will focus to only that selection.
...
viewof activeBubbleFilters = {
  ...
  let focusTopic = null; // New: Tracks if a single topic is "Solo Focused"
...
function resetChart() {
    focusTopic = null; // IMPORTANT: Clear the solo focus state
    focusedNodes.clear();
    dispatchChange();
    
    // Bring all subfields back
...
function onClick(event, d) {
  ...
  // HANDLE SOLO FOCUS (shift+Click)
  if (event.shiftKey) {
    event.preventDefault();
    // Toggle: if already focused, reset. Otherwise, focus this one.
    focusTopic = (focusTopic === d.id) ? null : d.id;
    
    // When solo focusing, we usually want the table to only show this topic too
    focusedNodes.clear();
    if (focusTopic) focusedNodes.add(focusTopic);
  ...
  if (d.type === "Subfield") {
      ...
      // If we were in Solo Focus mode and click another bubble, 
      // we probably want to break out of Solo Focus.
      focusTopic = null; 
...
function updateStyles() {
  nodeElements.selectAll("circle")
    ...
    .attr("fill", d => {
        const topicColor = d.type === "Subfield" ? "#0077c8" : colorScale(d.parentId);
        
        // 1. If Solo Focus is active (Shift+Click)
        if (focusTopic !== null) {
          // If it's the focused one, full color. If not, a very faint version of its OWN color
          return d.id === focusTopic ? topicColor : "#f1f5f9"; 
        }
        
        // 2. Standard Filtering View (Normal Click)
        if (focusedNodes.size === 0) return topicColor;
        return focusedNodes.has(d.id) ? topicColor : "#e2e8f0"; 
    })
    .attr("stroke", d => (focusTopic !== null && d.id === focusTopic) ? "#0077c8" : "#fff")
    .attr("stroke-width", d => (focusTopic !== null && d.id === focusTopic) ? 3 : 1)
    .attr("fill-opacity", d => {
       if (focusTopic !== null) return d.id === focusTopic ? 1.0 : 0.3;
       return 1.0;
    });
    
  nodeElements.selectAll("text")
    .attr("fill", d => {
      if (focusTopic !== null) return d.id === focusTopic ? "#ffffff" : "#94a3b8";
      return focusedNodes.size > 0 && !focusedNodes.has(d.id) ? "#94a3b8" : "#ffffff";
    })
    .style("opacity", d => (focusTopic !== null && d.id !== focusTopic) ? 0.4 : 1);
}
