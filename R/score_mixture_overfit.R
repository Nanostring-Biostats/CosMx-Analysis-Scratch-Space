...
score_mixture_overfit <- function(model, scores, prior_prob_level = NULL, denom_only = TRUE){
  ...
  p <- model$pihat
  ll <- lapply(1:nrow(model$mu_hat), function(k){
  ...
  colnames(post_probs) <- rownames(model$mu_hat)
