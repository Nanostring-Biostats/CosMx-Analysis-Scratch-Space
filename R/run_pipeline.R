...
#' 
#' @export
run_pipeline <- function(pipeline, counts_matrix, adjacency_matrix, initial_prior_weights = NULL
  ...
  for(parnt in parent_lists){
     metagene_scores[[parnt]]  <- 
       fit_metagene_scores(markerslist = pipeline$markerslist[[parnt]]
                           ,counts_matrix = counts_matrix
                           ,adjacency_matrix = adjacency_matrix
                           ,prior_level_weights = initial_prior_weights
                           ,...
                           )
     models[[parnt]] <- cluster_metagenes(metagenes = metagene_scores[[parnt]]
    ...
    for(chld in child_categories){
      prior_wts <- models[[pipeline$priors[[chld]]]]$post_probs[[pipeline$priors_category[[chld]]]]
       metagene_scores[[chld]]  <- 
         fit_metagene_scores(markerslist = pipeline$markerslist[[chld]]
                             ,counts_matrix = counts_matrix
                             ,adjacency_matrix = adjacency_matrix
                             ,prior_level_weights = prior_wts
                             ,...
                             )
       models[[chld]] <- cluster_metagenes(metagenes = metagene_scores[[chld]]
