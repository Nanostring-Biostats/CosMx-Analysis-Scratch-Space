...
#' @param quantile_absolute_expression_difference Quantile cutoff for absolute expression.  Default = 0.5. Both quantile cutoffs must be passed to retain gene.
#' @param quantile_percent_expression_difference Quantile cutoff for percent expression.  Default = 0.5. Both quantile cutoffs must be passed to retain gene.
...
supervisedSubcluster <- function(reference_profiles,
  ...
  nsprofiles <- reference_profiles[intersect(colnames(x), rownames(reference_profiles)),]
  nsprofiles <- apply(nsprofiles, 2, function(x) x/sum(x)*100) # normalize the profiles
  ...
  use_genes <- names(diffs)[diffs > quantile(diffs, quantile_absolute_expression_difference, na.rm = T) &
                             percs > quantile(percs, quantile_percent_expression_difference, na.rm = T)]

  # Remove genes that should not be considered.
  use_genes <- use_genes[!use_genes %in% excluded_genes]
  message("These are the selected ", length(use_genes),  " genes for subclustering:")
  dput(use_genes)

  # Warn if number of genes is low
  if(length(use_genes) < 300){
    warning(paste0("There are fewer than 300 genes being used to subcluster cells into ",
  ...
  if(!all(nonZeroCount_idx)) warning(paste0(sum(!nonZeroCount_idx), " cells have 0 counts in the subclustering gene panel."))
