...
#' @param quantile_percent_expression_difference_param
...
#' @examples
#'
#' test_celltype_relationship_list <- list(
#'   "structural" =
...
runInSituTree <- function(
  ...
  for (i in names(cth)) {
    ...
    message(i)
    # check to make sure further subtyping is required.
    # This is where cell lineages that are not present are dropped out.
    celltypes_dropped <- !i %in% unique(res$clust)
