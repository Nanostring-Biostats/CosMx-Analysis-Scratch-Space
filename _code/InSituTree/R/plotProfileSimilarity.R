...
plotProfileSimilarity <- function(full_profiles) {
  ...
  for (i in 1:n) {
    for (j in 1:n) {
      cosine_sim_matrix[i, j] <- cosine_similarity(full_profiles[, i],
                                                   full_profiles[, j])
    }
  }

  # Add names
  colnames(cosine_sim_matrix) <- colnames(full_profiles)
  rownames(cosine_sim_matrix) <- colnames(full_profiles)
