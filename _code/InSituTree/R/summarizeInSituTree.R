...
summarizeInSituTree <- function(nestList, annotationLevel = 1) {
  ...
  colnames(df_out) <- c(paste0("annotLevel_", annotationLevel),
                        paste0("probs_", "annotLevel_", annotationLevel))
  ...
  if (length(nestList$subclusterings) >= 1) {
    ...
    # copy over the higher level annotations for cells
    # that only have higher level annotations.
    for (i in 2:(ncol(df_out) / 2)) {
      if (any(is.na(df_out[, i * 2 - 1]))) {
        df_out[is.na(df_out[, i * 2 - 1]), i * 2 - 1] <-
          df_out[is.na(df_out[, i * 2 - 1]), i * 2 - 3]
        df_out[is.na(df_out[, i * 2]), i * 2] <-
          df_out[is.na(df_out[, i * 2]), i * 2 - 2]
      }
    }
  }
