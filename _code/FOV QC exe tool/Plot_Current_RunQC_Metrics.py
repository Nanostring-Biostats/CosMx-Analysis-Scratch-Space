...
def Plot_DZ_perSpot(x,y,dzFile, PlotDir):
    ...
    fig, axs = plt.subplots(sr, sc, figsize=(fig_width, fig_height))
    fig.suptitle('ZShift Distribution PerReporter; Max Shift:' + str(dzmax))
...
def Plot_Counts_perSpot(x,y,spotFile, PlotDir):
    ...
    noCols = len(cols)
    df = pd.read_csv(spotFile,usecols=[i for i in range(1,noCols)])
    ...
    plotFile = os.path.join(PlotDir,"SpotCounts_All_PerReporter.pdf")
    fig, axs = plt.subplots(sr, sc, figsize=(fig_width, fig_height))
...
def Plot_RawInt_perSpot(x,y,intFile, PlotDir):
    ...
    noCols = len(cols)
    df = pd.read_csv(intFile,usecols=[i for i in range(1,noCols)])
    ...
    # # # Plot raw intensity per Channel for each reporter
    rrcode=["BB","GG","YY","RR"]
    cbar_choice = [plt.cm.Blues, plt.cm.Greens, plt.cm.YlOrBr, plt.cm.Reds]
    for ch in range(4):
        plotFile = os.path.join(PlotDir,"RawSpotIntensity_"+rrcode[ch] +"_Per_Reporter.pdf")
        ...
        fig, axs = plt.subplots(sr, sc, figsize=(fig_width, fig_height))
        fig.suptitle(rrcode[ch] +' Raw Spot Intensity; Max Int:' +"{:.2f}".format(intMax) + "M")
        fig.tight_layout()
...
def Plot_All_RunQC_Metrics(RunSumDir):
    ...
    # # # # Plot the variance in Raw Spot Intensity as a function of reporter
    intFile =""
    for file in folder_content:
        if file.endswith('_RawSpotIntensity_perFOV.csv'):
            intFile = os.path.join(RunSumDir,file)
            break
    if(intFile==""):
        print("No _RawSpotIntensity_perFOV file found, skpping Raw Intensity per Reporter Plots...")          
    else:
        Plot_RawInt_perSpot(x,y,intFile, PlotDir)
        print('Completed Raw Intensity per Reporter Plots')
