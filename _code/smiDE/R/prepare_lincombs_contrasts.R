emmeans_to_lincombs <- function(linfct, inlanames, contrastnames){
  lc <- vector(mode='list',length=ncol(linfct))
  for(ii in 1:ncol(linfct)){
    nzwt <- which(linfct[,ii]!=0)
    lc[[ii]] <- 
      lapply(unname(nzwt)
             ,function(xx){
               ll=list(list(weight = unname(linfct[xx,ii])))
               names(ll)[1] <- inlanames[xx] 
               return(ll)
             })
    
  }
  names(lc) <- contrastnames 
  return(lc)
}
...
#' @export
#'
prepare_lincombs <- function(dat = NULL
                            ,fitmethod
                            ,original_formula
                            ,fam
                            ,inlanames
  ...
  onevrest_list <- list()
  allterms <- c()
  emm_lc_list <- ovr_lc_list <- pw_lc_list <- ova_lc_list <- list()
  ...
  famuse <- switch(fam
                   ,"gaussian" = "gaussian"
                   ,"poisson" = "poisson"
                   ,"nbinom2" = "poisson" ## just need log-link model to generate contrast matrices, use poisson for speed
  )
  linkused <- switch(famuse
                     ,"gaussian" = "identity" 
                     ,"poisson" = "log"
  )
  quickmod <- glm(original_formula, data=dat, family = famuse) 
  ...
  if(length(fixedterms) > 0){
    for(term in fixedterms){
      ...
      if(is.factor(dat[[term]])){
        moduse <- prepare_mod_emmeans(quickmod, term, "INLA::inla", fixed_formula, dat, cov_reduce_list=list())
        ...
        moduse <- prepare_mod_emmeans(quickmod, term, "INLA::inla", fixed_formula, dat, cov_reduce_list)
      ...
      if(length(levs) >= 2){
        ...
        emobdt <- merge(emobdt, levdt, by="level",sort=FALSE) 
        emobdt <- merge(emobdt, wtdt, by=c("level",intersect(fixedterms, names(wtdt))),sort=FALSE)
        if(emobdt[,.is.num(level)]) emobdt[,level:=paste0(term, "", as.character(level))]
        emm_lc <- emmeans_to_lincombs(t(emob@linfct), inlanames, emobdt[["level"]])
        ...
        if(length(levs) < 20*10**3){
          #browser()
          pw_contrast <- suppressWarnings(emmeans::contrast(emob, "pairwise",parens=NULL,adjust="none",data =dat))
          pw <- as.data.table(pw_contrast)
        ...
        if(length(levs) > 2){
          #browser()
          ovr_contrast <- suppressWarnings(emmeans::contrast(emob, "del.eff", wts = wts,parens=NULL,adjust="none", data = dat))
          onevrest <- as.data.table(ovr_contrast)
          ...
          ovr_lc <- emmeans_to_lincombs(t(ovr_contrast@linfct), inlanames, onevrest[["contrast"]]) 
          ...
          ova_contrast <- suppressWarnings(emmeans::contrast(emob, "eff", wts = wts, parens=NULL,adjust="none", data = dat))
          onevall <- as.data.table(ova_contrast)
          ...
          ova_lc <- emmeans_to_lincombs(t(ova_contrast@linfct), inlanames, onevall[["contrast"]]) 
          ...
          ovr_lc_list <- c(ovr_lc_list, ovr_lc)
          
          onevall_list[[term]] <- rbindlist(list(onevall_list[[term]]
                                                 ,onevall[,`:=`(term=term)]
          ))
          ova_lc_list <- c(ova_lc_list, ova_lc)
        }
        ...
        pw_lc <- emmeans_to_lincombs(t(pw_contrast@linfct), inlanames, pw[["contrast"]]) 
        pw_list[[term]] <- rbindlist(list(pw_list[[term]]
                                          ,pw[,`:=`(term=term)]
        ),use.names=TRUE,fill=TRUE)
        pw_lc_list <- c(pw_lc_list, pw_lc)
      }
      emobdt[,ky:=NULL]
      emm_list[[term]] <- rbindlist(list(emm_list[[term]]
                                         ,emobdt[,`:=`(term=term)]
      ))
      emm_lc_list <- c(emm_lc_list, emm_lc)
      #} 
    }
  } 
        ...
        if(length(levs) >= 2){
          ...
          for(jj in 1:length(wtdt_split)){
            ...
            if(length(levs) < 20 || (!is.null(groupVar) && term==groupVar)){
              pw_contrast <- suppressWarnings(emmeans::contrast(emob[emobdt_jj[,idx]], "pairwise",parens=NULL,adjust="none",data =dat, by = conditionvar))
              pw <- as.data.table(pw_contrast)
            }
            if(length(levs) > 2){
              ovr_contrast <- suppressWarnings(emmeans::contrast(emob[emobdt_jj[,idx]], "del.eff", wts = emobdt_jj[,wts],parens=NULL,adjust="none", data = dat, by = conditionvar))
              onevrest <- as.data.table(ovr_contrast)
              onevrest[,term_1:=gsub(" effect", "", contrast)] 
              onevrest[,contrast:=gsub("effect", "vs. avg.rest", contrast)]
              ovr_lc <- emmeans_to_lincombs(t(ovr_contrast@linfct), inlanames, onevrest[["contrast"]]) 
              ...
              ova_contrast <- suppressWarnings(emmeans::contrast(emob[emobdt_jj[,idx]], "eff", wts = emobdt_jj[,wts], parens=NULL,adjust="none", data = dat, by = conditionvar)) 
              onevall <- as.data.table(ova_contrast)
              onevall[,term_1:=gsub(" effect", "", contrast)] 
              onevall[,contrast:=gsub("effect", "vs. avg.all", contrast)]
              ova_lc <- emmeans_to_lincombs(t(ova_contrast@linfct), inlanames, onevall[["contrast"]]) 
  ...
  return(list(pairwise_list = pw_list
              ,onevrest_list = onevrest_list
              ,onevall_list = onevall_list
              ,emm_list = emm_list
              ,pairwise_lc_list = pw_lc_list
              ,onevrest_lc_list = ovr_lc_list
              ,onevall_lc_list = ova_lc_list
              ,emm_lc_list = emm_lc_list
              ,terms = allterms
  )) 
}