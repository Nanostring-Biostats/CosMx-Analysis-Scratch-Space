...
spatial_durbin_nnls_wrap <- function(y, X,signs,Wrs=NULL, wts=NULL, unconstrained = FALSE, lag_response = TRUE, lag_predictors = TRUE, training_ids = NULL){
  ...
  if(!is.null(wts)){
    if(is.null(names(wts))){
      names(wts) <- rownames(X)
    } else {
      stopifnot(all(names(wts) == rownames(X)))
    }
  ...
  if(!is.null(training_ids)){
    if(!is.null(wts)){
      a <- Matrix::crossprod(Matrix::Diagonal(x=sqrt(wts[training_ids])) %*% Xsigned[training_ids,])
      b <- Matrix::crossprod(Matrix::Diagonal(x=wts[training_ids]) %*% Xsigned[training_ids] , y[training_ids,,drop=FALSE])
    } else {
      a <- Matrix::crossprod(Xsigned[training_ids,])
      b <- Matrix::crossprod(Xsigned[training_ids,], y[training_ids,,drop=FALSE])
    }
  } else {
    if(!is.null(wts)){
