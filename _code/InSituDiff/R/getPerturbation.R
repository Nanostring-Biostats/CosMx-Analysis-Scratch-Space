...
#' @export
getPerturbations <- function(x, obj, cells = NULL, genes = NULL, modules = NULL, residtype = "log2ratio", eps = 1) {
...
scoreDiff <- function(env, controlenv, residtype, eps = 1) {
  ...
  if (residtype == "log2ratio") {
    return(log2(pmax(env, eps) / pmax(controlenv, eps)))     #<------------only works if 1 nearest neighbor is used
  }
  if (residtype == "diff") {
    return(env - controlenv)     #<------------only works if 1 nearest neighbor is used
  }
}
