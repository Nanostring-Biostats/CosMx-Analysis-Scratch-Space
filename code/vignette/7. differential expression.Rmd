...
For our example, we'll consider the question, "how do podocytes change gene expression
...
But we should remember that podocytes are probably responding to many factors in their environment, not just
...
*E(y) = f(number of nearby macrophages, local expression of inflammatory cytokines, number of nearby mesangial cells)*.

This model accounts for other immune impacts on podocyte expression and for any impact
of glomerulus composition on podocytes. (Podocytes live alongside mesangial cells in glomeruli.)
...
``` {r fastDE}
# subset on only podocytes:
inds <- clusts == "Podocyte"

# subset on only the analyzable genes \
# (those expressed above contamination as calculated above)
usegenes <- names(which([analyzable[, "Podocyte"]))
length(usegenes)

# get the design matrix to be used in every gene's model:
templm = lm(norm[inds, 1] ~ metadata$n_mac_neighbors[inds] + metadata$n_glom_endo_neighbors[inds] + metadata$n_lymphoid_neighbors[inds] + metadata$spatialclust[inds])
...
ses <- sqrt(diag(solve(t(X) %*% X)) * sigmasquared)
# and the p-value:
ps <- 2 * (1 - pt(abs(beta) / ses, df = nrow(X) - ncol(X)))
# convert from linear change in expression to a log2(fold-change):
log2ratios <- log2((beta0s + ests11) / beta0s)


## convince youself it reproduces lm():
# basic lm results:
summary(templm)$coef
...
if (FALSE) {
  ##### CODE FROM WNV ANALYSIS TO BE REPURPOSED (kept in script for now as record of starting math, to use to debug the above)
