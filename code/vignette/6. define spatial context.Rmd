...
# plot:
#sub <- sample(1:nrow(xy), 50000, replace = FALSE)
sub = TRUE
plot(xy[sub, ], pch = 16, cex = 0.8, 
     asp = 1, xlab = "", ylab = "",
     col = cols[as.numeric(as.factor(spatialclust[sub]))],
     xlim = c(4.5,6.5), ylim =c (6,8))
...
marks(pp) <- celltype
spatstat::marks(pp) <- as.factor(spatstat::marks(pp))
...
metadata$n_lymphoid_neighbors <- rowSums(neighbormarks[, c("T-cell", "B.cell", "NK.cell")])
```
