...
We find that a simple, computationally quick approach works well: 
we record the mean expression in each cell's neighborhood, and 
...
# cluster the top PCs:
nclust <- mclust::Mclust(pc, 
                         G = 8, # 8 clusters
                         modelNames = c("EVI")) 
spatialclust <- nclust$classification
...
## cluster it: 
nclust <- mclust::Mclust(neighbormarks, 
                         G = 8, # 8 clusters
                         modelNames = c("EVI")) 
spatialclust <- nclust$classification
...
annot$disttonearesttumor <- neighbor_dists$nn.dist[, 1]
# record 3rd-closest distance to the cell type:
annot$distto3rdnearesttumor <- neighbor_dists$nn.dist[, 3]
```
