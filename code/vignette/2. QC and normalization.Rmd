...
We check FOVs for two indicators of artifacts: lost efficiency, and biased gene expression profiles. 
Procedures for both these checks, along with a detailed discussion, can be found [here](https://nanostring-biostats.github.io/CosMx-Analysis-Scratch-Space/posts/fov-qc/).
These procedures will appear in AtoMx in summer 2024. 
Note that an additional approach to FOV QC, one looking at primary data accessible from AtoMx through S3, 
is described [here](https://nanostring-biostats.github.io/CosMx-Analysis-Scratch-Space/posts/FOV-QC-from-primary/).
...
## run the method:
res <- runFOVQC(counts = counts, xy = xy, fov = metadata$fov, barcodemap = barcodemap, max_prop_loss = 0.3) 
...
neighborcounts <- neighbor_mean(x = metadata$nCount_RNA, neighbors = neighbors)
...
FOVEffectsSpatialPlots(fovqc, outdir = NULL, bits = "flagged_reportercycles") 
