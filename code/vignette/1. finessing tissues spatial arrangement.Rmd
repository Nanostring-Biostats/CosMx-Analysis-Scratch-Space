...
metadata <- readRDS(paste0(mydir, "/processed_data/metadata.RDS")) # load cell metadata
```
...
# rescale to mm:
thisinstrument_nanometers_per_pixel = 120.280945   # this is the value for most instruments. Your RunSummary file will give the value for your instrument. 
...
plot(xy[sub, ], pch = 16, cex = 0.2, 
     ...
     col = as.numeric(as.factor(metadata$slidename[sub])))
# label tissues:
for (slidename in unique(metadata$slidename)) {
  text(median(xy[metadata$slidename == slidename, 1]), max(xy[metadata$slidename == slidename, 2]), slidename)
}
```
...
metadata$tissue <- NA
metadata$tissue[(metadata$slidename == "SMI0016c_SP2219") & (xy[, 2] > 8)] <- "sample1"
metadata$tissue[(metadata$slidename == "SMI0016c_SP2219") & (xy[, 2] < 8)] <- "sample2"
metadata$tissue[(metadata$slidename == "SMI0016c_SP212319") & (xy[, 2] > 8)] <- "sample3"
metadata$tissue[(metadata$slidename == "SMI0016c_SP212319") & (xy[, 2] < 8)] <- "sample4"
# check for hiccups:
...
xy <- condenseTissues(xy = xy, 
                      ...
                      widthheightratio = 2) # desired shape of final xy locations 
...
we can consolidate sample 2 a little better:

```{r consolidate}
isfarout <- xy[, 1] > 19
xy[isfarout, 1] <- xy[isfarout, 1] - 6
sub <- sample(1:nrow(xy), round(nrow(xy) / 20), replace = FALSE)
plot(xy[sub, ], pch = 16, cex = 0.2, 
     asp = 1, # important: keep the true aspect ratio
     col = cols[as.numeric(as.factor(metadata$tissue[sub])) %% length(cols) + 1])
...
```{r savexy}
saveRDS(xy, file = paste0(mydir, "/processed_data/xy.RDS"))
saveRDS(metadata, file = paste0(mydir, "/processed_data/metadata.RDS"))
```
