...
The [FastReseg R package](https://github.com/Nanostring-Biostats/FastReseg/tree/main) offers a rapid way to evaluate the performance of existing cell segmentation and to perform refinement given the spatial transcriptional profiles. 
...
You can see that cells with large `lrtest_nlog10P` value exhibited strong spatial dependency in their transcript score profiles under their best fitted cell type (i.e. `score_tLLR_maxCellType`). Below we would zoom in to one of the flagged cells (`c_1_51_238` with `lrtest_nlog10P = 10.91`) and visualize its transcript score profiles in 3D. 
```{ojs}
//| echo: false
...
viewof lowerLimit = Inputs.range(
  [Math.min(...data.map(d => d[colorColumn])), Math.max(...data.map(d => d[colorColumn]))], 
  { label: 'Lower Limit', 
    value: Math.min(...data.map(d => d[colorColumn])), 
    step: 0.1
  }
)
viewof upperLimit = Inputs.range(
  [Math.min(...data.map(d => d[colorColumn])), Math.max(0.0, Math.max(...data.map(d => d[colorColumn])))], 
  { label: 'Upper Limit', 
    value: Math.max(0.0, Math.max(...data.map(d => d[colorColumn]))), 
    step: 0.1
  }
)
```
...
createPlot(data, colorColumn, lowerLimit, upperLimit, plotDiv)
