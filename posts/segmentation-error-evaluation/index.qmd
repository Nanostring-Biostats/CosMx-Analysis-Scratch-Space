...
Many of those parameters have reasonable defaults for most spatial data sets and could be derived from your data using `FastReseg::runPreprocess()` function. For new user or new sample type, it's recommended to process one per-FOV transcript using `FastReseg::fastReseg_perFOV_full_process()` functions first and check out the impact of the parameters chosen. Please refer to [FastReseg tutorial](https://nanostring-biostats.github.io/FastReseg/articles/tutorial.html){target="_blank"}, `Modular functions for individual tasks` section, for more details. 
