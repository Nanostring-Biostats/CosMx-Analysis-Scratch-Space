...
Accurate cell segmentation that assigns transcripts to cell locations is critical to data quality of spatial transcriptomics assays, but very challenging for tissue sections where cells are tightly packaged with shared, 3D boundaries and uneven morphology staining. 

The [FastReseg R package](https://github.com/Nanostring-Biostats/FastReseg/tree/main) offers a rapid way to evaluate the performance of existing cell segmentation and to perform refinement given the spatial transcriptional profiles. The evaluation process starts with a cluster-specific reference expression profiles that are either derived from cell typing of query spatial dataset given its current image-based cell segmentation or from external non-spaital data sets, like scRNA-seq. Given the provided reference profiles, FastReseg algorithm scores individual transcripts for the goodness-of-fit within their respective cells based on the probability of each gene belonging to each cell type. FastReseg then scores each cell for its spatial dependency of transcript score profiles under its most likely cell type given the overall transcriptional profiles. As confirmed by the membrane-stained images, cells with boundary errors at the junction of different cell types, exhibit strong spatial dependency in their transcript score profile and thus can be easily identified. FastReseg further identifies the spatially connected groups of transcripts with low goodness-of-fit within incorrectly segmented cells. A set of heuristic rules on neighborhood cell typing and transcript number are then applied to the identified transcript groups to decide on the re-segmentation actions, like merging, splitting and trimming. The re-segmented results show no significant spatial dependency on transcript score of individual cells, suggesting the successful correction of poorly segmented cells.


![](figures/Fig1-FastReseg-workflow.png)


You can find the FastReseg package [here](https://github.com/Nanostring-Biostats/FastReseg/tree/main){target="_blank"}. See the corresponding [tutorial](https://github.com/Nanostring-Biostats/FastReseg/blob/main/vignettes/tutorial.html){target="_blank"} inside the package for more details. 


The required inputs for [FastReseg](https://github.com/Nanostring-Biostats/FastReseg/tree/main){target="_blank"} include:
...
This post will show you how to prepare your inputs if you have data assembled in the structure used by the Technology Access Program [TAP](https://nanostring.com/products/cosmx-spatial-molecular-imager/technology-access-program/); similar outputs are available from the AtoMx^TM^ Spatial Informatics Portal (SIP). You can download an example public dataset from [here](https://nanostring.com/products/cosmx-spatial-molecular-imager/ffpe-dataset/cosmx-smi-human-pancreas-ffpe-dataset/){target="_blank"}. The [ReadMe](./assets/Pancreas-CosMx-ReadMe.html) associated with this example data set on pancreas shows the data structures of each file used in this post.

- @sec-loading-flatFiles Prepare inputs from basic data files
- @sec-FastReseg-flagging Run FastReseg segmentation evaluation 
- @sec-FastReseg-full Run FastReseg full pipeline to correct putative segmentation error identified 
...
Here we start from the basic data files exported from AtoMx^TM^ SIP. 
...
all_rnas <- grep("fov|cell_ID|Negative|SystemControl", colnames(counts), value = TRUE, invert = TRUE)
...
We then load the transcript file which contains all molecules' coordinates and cell segmentation information. For faster processing in downstream `FastReseg` pipeline which is paralleled by input transcript files, we recommend to split the full transcript data into multiple files by FOV and export those per-FOV transcript information as individual csv files. We would pass their file paths to FastReseg function through `transDF_fileInfo` data.frame. 
...
fullTx <- fullTx[, .SD, .SDcols = c('transcript_id', 'cell', 'x_global_px', 'y_global_px', 'z', 'target', 'fov')]
...
transDF_fileInfo <- lapply(allFOVs, function(fovId){
  ...
  # since global coordinates of each molecule are available, use 0 for stage coordinates to disable convertion of local to global coordinates
...
`FastReseg` evaluates segmentation in physical space and thus it's preferred to convert all 3D coordinates into same unit. The relevant default parameters of `FastReseg` are set with respect to micrometer in coordinate unit. According to the  [ReadMe](./assets/Pancreas-CosMx-ReadMe.html), the pixel size for this data set is 0.12028 um per pixel and the z step size is 0.8um per z slice. We would pass this information to `FastReseg` functions to do the coordinate conversion. If your data is already in micrometer unit, you can use `1` for pixel size and z step to disable the conversion. 
...
<strong>1: In parallel::mclapply(X = seq_len(nrow(transDF_fileInfo)), mc.allow.recursive = TRUE,  :
...
# Run FastReseg segmentation evaluation {#sec-FastReseg-flagging}
...
```{r loadModStats, message=FALSE, eval = TRUE, echo=FALSE}
...
cells_to_plot <- cells_to_plot[seq(1, length(cells_to_plot), by = ceiling(length(cells_to_plot)/25))]
...
# Run FastReseg full pipeline to correct putative segmentation error identified {#sec-FastReseg-full}
...
There are several parameters one can adjust to tune the identification of wrongly segmented transcript groups and the rules used for cell segmentation correction. Please refer to [FastReseg tutorial](https://github.com/Nanostring-Biostats/FastReseg/blob/main/vignettes/tutorial.html){target="_blank"}, `Modular functions for individual tasks` section, for more details. 
