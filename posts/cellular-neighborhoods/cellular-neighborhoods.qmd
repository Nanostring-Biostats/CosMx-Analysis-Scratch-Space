...
Here's how you would get a neighborhood matrix with random subsetting:
...
fastSubsampleByRow <- function(neighbors, p = 0.5) {
  ...
  # pseudo-random vector of which neighbors to reset to 0:
  K <- sum(neighbors[1, ] != 0)
  keepofK <- rep(0, K)
  keepofK[sample(1:K, round(K * p))] <- 1
  keep <- rep(keepofK, nrow(neighbors))
  # update elements of neighbors matrix:
