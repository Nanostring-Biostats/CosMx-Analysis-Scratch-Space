...
description: "This post discusses batch effects in CosMx; how they impact analyses, how to detect them, and how to correct them."
draft: false
---
...
saveRDS(list(pca_head = pcaobj@cell.embeddings[1:100,], pca_str = capture.output(str(pcaobj@cell.embeddings))), file="figures/pcaobj.embeddings.str.rds")
```
...
saveRDS(list(harmony_head = harmony_embeddings[1:100,], harmony_str = capture.output(str(harmony_embeddings))), file="figures/harmony.embeddings.str.rds")
...
The `harmony_embeddings` returned are a `cells`  $\times$ `# of pc's` dimension matrix, with the same structure as the PCA embeddings.
...
pca_head <- readRDS("figures/pcaobj.embeddings.str.rds")
harmony_head <- readRDS("figures/harmony.embeddings.str.rds")

cat(harmony_head$harmony_str, sep="\n")

print(harmony_head$harmony_head[1:10,1:5])
```


Here are what the original PCA embeddings look like for comparison:

```{r}
#| eval: false
#| echo: true

str(sem_comb[["pca"]]@cell.embeddings)
sem_comb[["pca"]]@cell.embeddings[1:10,1:5]
...
cat(pca_head$pca_str, sep="\n")

print(pca_head$pca_head[1:10,1:5])
...
set.seed(123)
shuffle_order <- sample(1:nrow(harmony_embeddings)) ## shuffle cell order for plotting
cls <- c("#1f77b4", "#ff7f0e")
names(cls) <- c("v2.0", "v1.5")
...
harmony_plt <- 
ggplot(pc1vs2_harmony[shuffle_order], aes(x = PC_1, y = PC_2, color=batchid)) + geom_point(size=0.2, alpha=0.5, shape=16) + 
...
regular_pca_plt <- 
ggplot(pc1vs2_regular[shuffle_order], aes(x = PC_1, y = PC_2, color=batchid)) + geom_point(size=0.2, alpha=0.5, shape=16) + 
  scale_color_manual(values = cls) + 
  theme_bw() + 
  coord_fixed() + 
  guides(color = guide_legend(override.aes=list(size=4, alpha=1)))
...
ggsave(filename = "figures/thumbnail.png"
       ,plot = pc_comparison_plt
       ,width = 1704/3.5
       ,height = 848/3.5
       ,units = 'px'
...
harmony_plt <- 
ggplot(umap_harmony[shuffle_order], aes(x = UMAP_1, y = UMAP_2, color=batchid)) + geom_point(size=0.2, alpha=0.5, shape=16) + 
  scale_color_manual(values = cls) + 
  theme_bw() + 
  coord_fixed() + 
  guides(color = guide_legend(override.aes=list(size=4, alpha=1)))
...
regular_umap_plt <- 
ggplot(umap_regular[shuffle_order], aes(x = umap_1, y = umap_2, color=batchid)) + geom_point(size=0.2, alpha=0.5, shape=16) + 
  scale_color_manual(values = cls) + 
  theme_bw() + 
  coord_fixed() + 
  guides(color = guide_legend(override.aes=list(size=4, alpha=1)))
