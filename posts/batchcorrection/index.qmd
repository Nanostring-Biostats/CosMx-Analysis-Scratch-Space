...
For example, suppose I were interested in analyzing multiple breast cancer tissue samples, but these samples were processed in different labs, by different scientists, with different sample prep protocols. These technical differences could subtly effect gene detection efficiency in ways that may influence or confound an analysis.

To check for presence of batch effects and whether correction is required, we recommend plotting the 'batch' on a UMAP or in principal component space [](#pcplotbatch).  In the presence of batch effects, we may observe that cells cluster together by batch more distinctly than they do by their biological cell type. 

For dimension reduction and clustering analyses, there are a multitude of existing research on batch correction methodology [@luecken2019current].  Harmony [] has been recommended as a best-in-class approach in several studies [@tran2020benchmark] [@emmanuel2024batch] which benchmarked different batch correction methodology for scRNAseq. 
...
There is a multitude of existing research on batch correction methodology as well as methods implemented in popular packages like Seurat [link](https://satijalab.org/seurat/articles/integration_introduction) . 

In this post, we demonstrate batch effect correction using Harmony on two 6k-plex tonsil tissue datasets; one dataset generated with CosMx 1.5, while the other generated with CosMx 2.0.  The recent CosMx 2.0 release offers noticeable increases in sensitivity; while sensitivity increases will vary from sample-to-sample, transcripts per cell often increase 1.5-2x compared to the same sample processed with previous CosMx 1.5 software.  In case we want to analyze a v1.5 and v2.0 dataset in the same analysis, we might consider treating this variable as a batch effect. 




to analyze multiple flow cells of the same tissue type.  


Here, we demonstrate the effectiveness of one of these Harmony [paper link](https://pmc.ncbi.nlm.nih.gov/articles/PMC6884693/) [@harmony], and show  

Here we demonstrate how one could use Harmony for batch correction.  We start with two comparable sections  of 6k-plex tonsil tissue.  
...
May depend on both the scope and depth of the analysis.
