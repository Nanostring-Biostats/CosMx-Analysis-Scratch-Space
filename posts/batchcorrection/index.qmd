...
For example, suppose I were interested in analyzing multiple breast cancer tissue samples, but these samples were processed in different labs, by different scientists, or with different sample prep protocols. These technical differences could subtly effect gene detection efficiency in ways that may influence or confound an analysis.

To check for presence of batch effects and whether correction is required, we recommend [plotting](#pcplotbatch) the 'batch' on a UMAP plot or in principal component (PC) space.  In the presence of batch effects, we may observe that cells cluster together by batch more distinctly than they do by their biological cell type. 
...
In this post, we demonstrate using Harmony for batch effect correction on two 6k-plex tonsil tissue datasets; one dataset generated with CosMx 1.5, while the other generated with CosMx 2.0.  The recent CosMx 2.0 release offers noticeable increases in sensitivity; while sensitivity increases will vary from sample-to-sample, transcripts per cell often increase 1.5-2x compared to the same sample processed with previous CosMx 1.5 software.  In case we want to analyze a v1.5 and v2.0 dataset in the same analysis, we might consider treating this variable as a batch effect. 
