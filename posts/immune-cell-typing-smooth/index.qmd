...
1. First, for each cell type, we define a set of genes we expect might to be 'marker genes' or highly expressed based on prior data or biology (either the literature, a reference dataset, or some combination of both).
...
1. Defining sets of genes we expect might be expressed for each cell type based on prior data or biology (literature or a reference dataset).
  * Can be defined based on a reference dataset, or literature.  
  * Expect that some  (but not all) 
2. Constructing metagene scores for that celltype based on the genes specified in (1).
3. Clustering metagene scores from step into and classifying cells.




1. Ability to flexibly encode our assumptions based on prior knowledge of the biology and the literature.
2. Make cell type classification robust to common challenges faced with SRT data.
  -- variables producing metagene scores act as 'instruments'.  In general uncorrelated with segmentation error in a particular marker gene
  -- flexibly model cells which are double-positive for multiple categories and re-assign them 
