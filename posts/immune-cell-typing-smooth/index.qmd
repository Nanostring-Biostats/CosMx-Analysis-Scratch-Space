...
Code for the method and utility functions used in this post can be found in the R package [HieroType](https://github.com/Nanostring-Biostats/CosMx-Analysis-Scratch-Space/tree/Main/_code/HieroType). 
...
normed <- HieroType::totalcount_norm(Matrix::t(sem[["RNA"]]@counts))
...
devtools::load_all("~/data/dan/imputation_smoothing/HieroType", export_all = FALSE)
```
...
lapply(split(data.table(sem@meta.data), by = "portion"), function(xx){
  HieroType::xyplot("clusters_unsup", metadata = xx,ptsize = 0.1) + coord_fixed()
})
xypl <- 
...
Here we can use a couple of convenience functions provided in the R package `HieroType`.

```{r, eval=FALSE}
#| code-fold: true

fctbl_unsup <- 
HieroType::clusterwise_foldchange_metrics(normed  = sem[["RNA"]]@data
                                          ,metadata = sem@meta.data
                                          ,cluster_column = "clusters_unsup"
                                          )

hm_unsup <- HieroType::marker_heatmap(fctbl_unsup
...
Here's the code for classifying leiden cluster '6' into `cd8t` and `cd4t` cell types , followed by more granular T cell subtyping.  We use the R package `HieroType` with a prespecified `tcell_pipeline`, and corresponding lists of marker genes included as package datasets.
...
model_t <- 
HieroType::cluster_metagenes(metagenes = metagenes_t)
```
...
```{r, eval=TRUE}
str(HieroType::markerslist_cd4tminor, max.level=2)
```

```{r, eval=FALSE}
#### Classify cd4t subtypes
metagenes_t4 <-
  HieroType::fit_metagene_scores(
    markerslist = HieroType::markerslist_cd4tminor
    ,counts_matrix = Matrix::t(sem[["RNA"]]@counts[,tcell_ids])
    ,adjacency_matrix = as(sem@graphs$umapgrph
                           ,"CsparseMatrix")[tcell_ids,tcell_ids]
...
model_t4 <- 
HieroType::cluster_metagenes(metagenes = metagenes_t4, prior_prob_level = model_t$post_probs$cd4t)
...
```{r}
pipeline_tcell <- 
  make_pipeline(markerslists = list("tmajor" = HieroType::markerslist_tcellmajor
                    ,"t4minor" = HieroType::markerslist_cd4tminor
                    ,"t8minor" = HieroType::markerslist_cd8tminor
                    )
...
```{r}
str(HieroType::pipeline_tcell, max.level=2)
```
...
```{r}
HieroType::combine_postprob_tables(pipeline = pipeline_tcell
...
idxs <- unique(unname(c(unlist(lapply(HieroType::markerslist_cd4tminor, "[[", "index_marker"))
               ,unlist(lapply(HieroType::markerslist_cd8tminor, "[[", "index_marker")))))

nms <- c("cd4t", names(HieroType::markerslist_cd4tminor), "cd8t", names(HieroType::markerslist_cd8tminor))
...
lapply(HieroType::markerslist_cd4tminor, "[[", "index_marker")

### The key index markers for cd8 subtypes
lapply(HieroType::markerslist_cd8tminor, "[[", "index_marker")
```
...
hm_tcell <- HieroType::marker_heatmap(fc_t_all
...
pipeline_io <- HieroType::make_pipeline(
...
```{r}
str(HieroType::pipeline_io,max.level=3)
```
...
```
```{r}
lapply(HieroType::markerslist_immunemajor, "[[", "index_marker")
...
xyplist_sup <- 
  lapply(split(met, by="portion"), function(xx){
    HieroType:::xyplot(cluster_column = "celltype_supervised", metadata = xx
...
Primarily,.   For cases where cell types exist in a dataset, but don't have , the. Along this same line
...
We've implemented the tools presented here in the R package [HieroType](https://github.com/Nanostring-Biostats/CosMx-Analysis-Scratch-Space/tree/Main/_code/HieroType).

# Quick summary of package functions used in this post

Below is a short list of primary package functions which are used throughout this post.
...
* `metagene_pairsplot()` - Plot for pairwise comparisons of metagene scores.
* `metagene_coefficients_plot()` - Plot for which genes contribute most to cell type metagene scores.
* `clusterwise_foldchange_metrics()` - Computes
* `marker_heatmap()`
