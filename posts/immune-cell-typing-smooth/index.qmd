...
We use a 6k colon-cancer dataset to demonstrate two common use cases for the method:
...
sem <- Seurat::AddMetaData(sem, metadata = data.frame(immune_typing$post_probs$l1[,.(celltype_granular)], row.names = immune_typing$post_probs$l1$cell_ID))
umapf("umap", "celltype_granular", semuse = sem, alpha = 0.5)
umapf("umap", "clusters_unsup", semuse = sem, alpha = 0.5)
comp <- 
merge(
  fc_l1[gene %in% c("CD3D", "CD3E", "CD3G","IL7R","MS4A1")][cluster %in% c("cd4t", "cd8t")][
    ,.(ncells=sum(ncells),cluster_expr = sum(cluster_expr*ncells)/sum(ncells)),by=.(gene)]
  ,fc_l1[gene %in% c("CD3D", "CD3E", "CD3G","IL7R","MS4A1")][!cluster %in% c("cd4t", "cd8t")][
    ,.(ncells_prime=sum(ncells),cluster_expr_prime = sum(cluster_expr*ncells)/sum(ncells)),by=.(gene)]
  ,by="gene")[,fold_change:=cluster_expr / cluster_expr_prime]

fctbl_unsup[gene %in% c("CD3D","CD3G", "CD3E", "IL7R","MS4A1")][cluster==6]
fc_l1[gene %in% c(smiSmooth::markerslist_immunemajor$bcell$index_marker)][cluster %in% c("bcell")][
  order(-fold_change)]
