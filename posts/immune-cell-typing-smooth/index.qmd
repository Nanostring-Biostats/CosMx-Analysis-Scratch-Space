...
This post demonstrates a cell typing method optimized for detection and sub-clustering of immune cells.  Often (particularly with immune cells), we may want our cell type calls to be guided by specific sets of marker genes.  
In some spatially-resolved transcriptomics (SRT) datasets, directly using marker gene expression for determining cell type may be a challenge due to lower sensitivity compared to scRNAseq data, background due to autoflourescence, or segmentation ambiguity.

The cell typing method we describe here is designed to make it easy to enforce prior biological assumptions about the marker genes and expression profiles we expect to see in our cell types, while being robust to platform or batch effects that may otherwise challenge the transfer of those assumptions from pre-trained models or reference datasets.  
...
For anyone in a hurry to 'just see the code', they can jump ahead to the "TL;DR 'just the code'" sections for [Example 1](#tldr), or [Example 2](#tldr2).
...
<div class="wider-image">
  <img src="./figures/hm_tcells_v1_annotated.png" alt="Wide Image">
</div>
```{r}
#| eval: false
#| echo: false

knitr::include_graphics("./figures/hm_tcells_v1_annotated.png")
```
...
Looking at the `markerslist_l1io` object below, we've specified "PTPRC" (the CD45 gene) as a primary index marker for `immune` cells. One strategic point of note here is that we treat `plasma` as it's own category for the purposes of this algorithm, rather than a subtype of `immune`; this is because plasma cells often lose CD45 as they differentiate and mature in their transition from B cells, and so are easier to identify when kept as a distinct category.
