...
Spatial Informatics Platform (SIP) are provided as multi-channel TIFF 
...
Bruker Spatial Biology. We do not (yet) have the source code for `napari-cosmx` opened-source.
...
is available as a `whl` file in the assets folder of the [Scratch Space repository](https://github.com/Nanostring-Biostats/CosMx-Analysis-Scratch-Space/blob/Main/assets/napari-cosmx%20releases/napari_CosMx-0.4.17.3-py3-none-any.whl). 
...
Note that the segmentation (`-s`) layer does not count toward the batch size. In other
...
Download and install `vips-dev-w64-all-8.16.0.zip` from [here](https://github.com/libvips/build-win64-mxe/releases/)
