...
each IF or protein image. 

For visualization of tissue microscopy data, 
[OME-TIFF](https://docs.openmicroscopy.org/ome-model/5.6.3/ome-tiff/) is a widely used format. The Zarr stores generated with `napari-cosmx` can be converted to OME-TIFF for viewing in other appolications. This short blog post demonstrates how to perform this conversion.

This post is the fifth in our [`napari series`](https://nanostring-biostats.github.io/CosMx-Analysis-Scratch-Space/#category=napari). 
...
The size of the OME-TIFF file, which contains 5 IF channels plus the segmentation layer, 
is 54 GBs (_i.e._, about 91% of the size of the Zarr store). 

The default method to convert Zarr to OME-TIFF can require more memory than your
system has. If you run memory issues, you can try the secondary method that uses
`pyvips` (see @sec-appendix).
