...
Marker genes are genes that are expressed primarily within a single-cell type, and are often used to delineate and name clusters during cell typing in a scRNAseq analysis.

In spatial transcriptomic (ST) data, lower sensitivity, autoflourescent background, and segmentation error, can contribute to challenges in cell typing analyses, as well as hinder our ability to visualize our favorite marker genes.

A common feedback for new ST analysts when dealing with ST data may be along the lines of "Why don't I see gene 'X' in a majority of cells for cell type 'Y'?".  

This post demonstrates a method for calculating the smoothed expected expression of any gene, conditioning on it's most similar neighbor cells.  Neighbor cells here are defined based on local proximity in UMAP space. 
...
Let's load it in, run some quick unrefined-cell typing using InSituType (https://www.biorxiv.org/content/10.1101/2022.10.19.512902v1.full.pdf), and take a look at the initial results.
...
This UMAP was created using 'Analytic Pearson residuals' (https://genomebiology.biomedcentral.com/articles/10.1186/s13059-021-02451-7 ) .
...
- CD8 (used together with CD3 to identify T CD8 cells)

First, let's take a look at the relative frequency of our T cell clusters 'Treg', 'T CD8', and 'T CD4'.
Tregs are supposed to be a particular sub type of T CD4 cells. Yet we have more than 3x as many Tregs called than we do T CD4.  
...
smoothed_tcell_markers <- sem[["RNA"]]@data[c("FOXP3","CD3G","CD8B"),] %*% smoother
...
met[["foxp3smooth"]] <- marker_matrix["FOXP3",met[["cell_ID"]]]
