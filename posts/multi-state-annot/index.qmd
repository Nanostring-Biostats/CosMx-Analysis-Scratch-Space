...
/* Compact layout for wide metadata tables with one long description column. */
.table-scroll .table.compact-table th,
.table-scroll .table.compact-table td {
...
.table-scroll .table.program-sets-table,
.table-scroll .table.tumor-summary-table {
  table-layout: fixed;
}

.table-scroll .table.program-sets-table th:not(:nth-child(7)),
.table-scroll .table.program-sets-table td:not(:nth-child(7)) {
  width: 8.5em;
  max-width: 8.5em;
  overflow-wrap: anywhere;
}

.table-scroll .table.program-sets-table th:nth-child(7),
.table-scroll .table.program-sets-table td:nth-child(7) {
  min-width: 28em;
  width: 28em;
  max-width: 36em;
  white-space: normal;
...
.table-scroll .table.tumor-summary-table th:not(:nth-child(5)),
.table-scroll .table.tumor-summary-table td:not(:nth-child(5)) {
  width: 10em;
  max-width: 10em;
  overflow-wrap: anywhere;
}

.table-scroll .table.tumor-summary-table th:nth-child(5),
.table-scroll .table.tumor-summary-table td:nth-child(5) {
  min-width: 30em;
  width: 30em;
  max-width: 42em;
  white-space: normal;
...
.table-scroll table.compact-table th:first-child,
.table-scroll table.compact-table td:first-child {
  position: sticky;
  left: 0;
...
/* Freeze header row on vertical scroll */
.table-scroll table.compact-table thead th {
  position: sticky;
  top: 0;
  background-color: white;
  z-index: 11;
...
/* Top-left corner cell (frozen row + frozen column) gets highest z-index */
.table-scroll table.compact-table thead th:first-child {
  z-index: 12;
}
</style>
...
The tumor/auxiliary track was designed primarily for tumor-status annotation and tumor-like label integration (which integrates both lineage-based cell type annotation and descriptive tumor status annotation to final export label of a tumor-positive cluster, example shown in @tbl-intgr-annot). The same machinery can also be used in `flag_only` mode for non-tumor auxiliary programs such as cell cycle, stress, inflammation, or fibrosis.

This post focuses on the tumor/auxiliary program track. I use malignant status annotation as the primary example, but the same design supports broader multi-state annotation. At the end of this workflow, `HierAnnot` would annotate clusters with labels that carry the auxiliary status description (@tbl-cluster-malig) in addition to the lineage-based cell type labels (@tbl-cluster-annot). 
...
::: {#nte-combined-scoring .callout-note collapse="true" title="Under the hood: combined malignant-status scoring"}
...
# Running pipeline with auxiliary track

::: {.callout-tip title="Just the code"}
For a quick start with just the code, refer to the [examples scripts](https://github.com/Nanostring-Biostats/CosMx-Analysis-Scratch-Space/tree/Main/_code/HierAnnot/examples){target="_blank"} under `HierAnnot/examples` subfolder of source code repository. 
...
## Setup hierarchy and auxiliary marker program set
...
# scrollable table with wider width for the 7th description column
cat('<div class="table-scroll">')
print(knitr::kable(df, format = "html", table.attr = 'class="table table-striped table-hover compact-table program-sets-table"'))
cat("</div>")
```

Each built-in marker program set is designed for different annotation purpose and `HierAnnot` allows you to pick one marker program set at a time. For the colon cancer example dataset here, we will use a colon tissue microenvironment hierarchy `colon_tme` and a colon-specific tumor program set `tumor_colon`. Please refer to package [README](https://github.com/Nanostring-Biostats/CosMx-Analysis-Scratch-Space/tree/Main/_code/HierAnnot#built-in-and-custom-malignant-programs){target="_blank"} on how to customize the marker program set (i.e. a custom list of `MalignantProgram` objects) for auxiliary track annotation.
...
# scrollable table with wider width for the 5th description column
cat('<div class="table-scroll">')
print(knitr::kable(df, format = "html", table.attr = 'class="table table-striped table-hover compact-table tumor-summary-table"'))
cat("</div>")
```
...
Within the built-in `tumor_*` program sets, `core_transformation` is the main generic tumor-status module, which captures compact expression evidence for a **transformed**, tumor-like **epithelial** state that can be evaluated across solid-tumor data sets. `HierAnnot` treats this as **tumor-status** evidence: _if the program is sufficiently enriched and supported by detected markers, it can contribute to a tumor-like call_. More specific programs, such as EMT, cycling, stress, or tissue-specific melanocytic programs, are then used as **state** or **modifier** evidence _which do not establish tumor-like identity by themselves but add a small support bonus only when core tumor-status evidence is already borderline_. See details on program roles and tumor-status scoring in @nte-combined-scoring above.
...
#| tbl-cap: "Marker table for `colon_tme` hierarchy and built-in marker program sets for `tumor_colon` and `stress`."
