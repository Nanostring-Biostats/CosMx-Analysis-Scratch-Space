...
* **H\&E image**: Example file path at `~/data/cosmx/WTx_Exp-25_Colon_HE.png`. Napari supports various other image formats, including `TIFF`and `Zarr`. 
...
1. Start `napari`. 
2. Load H\&E: Drag the H\&E file into `napari` GUI (it will be a 2D image layer), choose `napari builtins` reader if prompt.
3. Load CosMx dataset: Drag the pre-stitched cosmx data folder (which contents morphology image, cell segmentation, cell‑type metadata, etc.) into `napari` GUI, choose `napari CosMx` reader if prompt. 
