...
1. How to numerically represent cells' "spatial contexts" (output: a matrix of cells x variables encoding details of cells' surroundings)
2. How to partition a tissue or study into discrete spatial domains / niches

The second problem has been a focus of spatial data analysis since the very beginning - 
...
The first problem - that of encoding spatial context - is usually solved in service of domain detection, 
...
Lately we favor the Novae algorithm, which uses a spatial transformer in place of our steps 1-2 and uses a well-crafted hierarchical clustering method in place of our step 3. 
Novae produces lovely, spatially-nuanced domains, but it is relatively inconvenient to stand up, is slow without a GPU, and often needs computationally expensive fine-tuning. 
...
colorByMatrix <- function(mat, maxquant = 0.01, na.color = "#808080", ...) {
...
envcols <- colorByMatrix(mat = env, maxquant = 0.01, na.color = "#808080") 
