---
title: "Using Squidpy with AtoMx<sup>&#174;</sup> SIP exports"
author:
  - name: Evelyn Metzger
...
description: In this blog post, I’ll show how to prepare and analyze AtoMx<sup>&#174;</sup> SIP-exported CosMx<sup>&#174;</sup> SMI data with python\'s squidpy package.
format: 
  html:
    theme: 
      light: custom.scss
...
of the fundamental utilities of the [CosMx<sup>&#174;</sup>](https://nanostring.com/products/cosmx-spatial-molecular-imager/single-cell-imaging-overview/){target="_blank"} Spatial Molecular Imager (SMI).
When it comes to analysis, the [AtoMx<sup>&#174;</sup>](https://nanostring.com/products/atomx-spatial-informatics-platform/atomx-sip-overview/){target="blank"}
...
ways to visualize tissue images built in. More recently, Seurat [@Hao2024], another R
package (and another artist!), has added image viewing capabilities into its workflow. For vignettes on how to analyze
and visualize SMI data with Seurat, check out [our recent blog post](../seurat-cosmx-basics/index.qmd){target="_blank"} or
...
For more information on the differences, see [this post](../flat-file-exports/flat-files-compare.qmd).
...
# Exporting data from AtoMx<sup>&#174;</sup> SIP {#sec-export}

For this post I will be analyzing a single breast cancer slide that has about 80,000 cells, 
64 FOVs, and with the [6K Discovery Panel](https://nanostring.com/products/cosmx-spatial-molecular-imager/cosmx-rna-assays/human-6k-discovery-panel/). I'm using the CosMx Data Analysis v1.3.2 software in AtoMx SIP.
...
raw data but rather processed from the raw data. Versions of flat files may change with time
and over the course of an analysis (_e.g._, cell type column added to the metadata file as highlighted in [this post](../flat-file-exports/flat-files-compare.qmd)).
:::
...
:::{.callout-note}
The column names that you have may differ from the ones above. That's because the example
dataset has been processed in AtoMx SIP which created additional columns (_e.g._, RNA_nbclust_[GUID]_1_clusters). For
more information on the expected columns from flat file exports, see 
the [metadata column descriptions](../flat-file-exports/flat-files-compare.qmd#sec-meta). 
:::

## Including the FOV file

If we try to read in the optional FOV file generated in AtoMx SIP using squidpy v1.5.0, we might get an error
...
This error arises because of a format change that is detailed in [this post](../flat-file-exports/flat-files-compare.qmd#sec-fov-pos). A simple fix is to adjust the column name in the fov file from "FOV" to the legacy "fov" like this:
...
code in this section is an example and not a recommendation for specific thresholding, 
...
reflect the flat file format changes ([detailed here](../flat-file-exports/flat-files-compare.qmd#sec-fov-pos)). 
...
the [`seaborn`](https://seaborn.pydata.org/){target="_blank"} package. For example the code below generates @fig-qc and shows
...
Histograms of total transcripts, total unique genes per cell, and total transcrips per FOV can be seen in @fig-qc2.
```{filename="Python"}
...
We can filter, normalize, and cluster with scanpy's functions. For a full list
of available functions, see scanpy's [API documentation](https://scanpy.readthedocs.io/en/latest/api/index.html){target="_blank"}.
...
#| label: "fig-umap"
#| fig-width: 4
#| fig-cap: "UMAP figures with cells colored by natural log Mean PanCK expression (left) and Leiden clusters (right)."
...
to prepare a slide exported from AtoMx SIP into squidpy, and touch on some of the
...
The block of code below reads the flat files _and images_ now that the image data are in the
specified format. The location of the data in the example code below is `sample_dir`
and is provided as the argument for the `path` parameter of `read.nanostring`.
...
For some of the examples below, I'll use a subset of the data -- a single FOV -- to 
...
#| label: "fig-image1"
#| fig-width: 4
#| fig-cap: "Leiden clusters in space for a single FOV of breast cancer tissue. Clusters are displayed as cell contours overlaid on morphology image (green = PanCK, magenta = DAPI). "
...
While Moran's _I_ quantifies autocorrelation of genes, spatial co-occurence with
`gr.co_occurence` quantifies cell-level co-occurences. In example below, co-occurence
of Leiden clusters was used (@fig-co-occurence).
...
#| fig-width: 8
#| fig.show: 'hold'
#| fig-cap: "Co-occurence of Leiden clusters. Left: Leiden clusters in space for a single FOV of breast cancer tissue. Right: co-occurence probability ratio (Y) for each cluster relative to Leiden cluster 1 (orange cells in left plot) with increasing radius (X)."

knitr::include_graphics("./figures/fig-co-occurence1and2.png")
```
...
In this post I showed how to pivot CosMx SMI data that were exported from AtoMx SIP
into a format that can be read with the open-sourced python package squidpy. A full
analysis of SMI data in squidpy was beyond the scope of this post but look for additional
blog post that utilize squidpy's functions.
...
:::{.callout-note}
Troubleshooting or assistance with squidpy installation is beyond the scope of this post. 
Please see [squidpy's documentation](https://squidpy.readthedocs.io/en/stable/api.html){target="_blank"}.
:::
