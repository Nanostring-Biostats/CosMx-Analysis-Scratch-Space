...
- @sec-export export the squidpy-relavent files
...
for some of the files (error not shown). So for simplicity of this post, I will make a copy of the 
...
This issues patched for samples created in v1.3.2+. If you have blank images and
...
sample_dir = '/Volumes/Extreme_Pro/data/Demo_AGBT_6K_Breast_19_06_2024_13_37_37_539/sample_dir_formatted'
...
single-FOV subset, we genes with positive Moran's _I_. When we plot the expression
of some of these genes (@fig-morans), we indeed see some how they are aggregated. 
For example, _COX6C_, which have clinical significance with breast cancer [@Tian2021],
...
## Comparison between legacy formats and AtoMx 1.3.2 format for fov_positions {#sec-comparison}

In this section I compare the flat files created for `Lung5_rep1` of the NSCLC dataset
side-by-side with the updated (AtoMx 1.3.2) flat file format for fov_positions. The
specific values will be different since they are different datasets but the side by
sides should showj
...
fov_file_old <- read.csv("/Volumes/Extreme_Pro/data/public_data/NSCLC/Lung5_Rep1/Lung5_Rep1-Flat_files_and_images/Lung5_Rep1_fov_positions_file.csv", header=TRUE)
head(fov_file_old, 10)
```

```
   fov x_global_px y_global_px
1    1    3188.889    155216.7
2    2    8661.111    155216.7
3    3   14133.333    155216.7
4    4   19605.556    155216.7
5    5   25077.778    155216.7
6    6    3188.889    158866.7
7    7    8661.111    158866.7
8    8   14133.333    158866.7
9    9   19605.556    158866.7
10  10   25077.778    158866.7
```
...
fov_file_new <- read.csv("/Volumes/Extreme_Pro/data/Demo_AGBT_6K_Breast_19_06_2024_13_37_37_539/sample_dir_formatted/AUG29_13INTEGR_6K_BRST_PS_S2_fov_positions_file.csv.gz", header=TRUE)
head(fov_file_new, 10)
```

```
   FOV x_global_px y_global_px x_global_mm y_global_mm
1    1           0       29791   0.0000000    3.583410
2    2        4255       29791   0.5119157    3.583410
3    3        8511       29791   1.0238314    3.583410
4    4       12767       29791   1.5357471    3.583410
5    5       17023       29791   2.0476628    3.583410
6    6       21279       29791   2.5595785    3.583410
7    7       25535       29791   3.0714942    3.583410
8    8       29791       29791   3.5834099    3.583410
9    9           0       25535   0.0000000    3.071494
10  10        4255       25535   0.5119157    3.071494
```
...
library(plyr)
library(dplyr)
expr_file_old <- read.csv("/Volumes/Extreme_Pro/data/public_data/NSCLC/Lung5_Rep1/Lung5_Rep1-Flat_files_and_images/Lung5_Rep1_exprMat_file.csv", header=TRUE)
...
expr_file_new <- read.csv("/Volumes/Extreme_Pro/data/Demo_AGBT_6K_Breast_19_06_2024_13_37_37_539/sample_dir_formatted/AUG29_13INTEGR_6K_BRST_PS_S2_exprMat_file.csv.gz", header=TRUE)
