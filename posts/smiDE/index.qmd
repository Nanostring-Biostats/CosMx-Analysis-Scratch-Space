...
Led by the brilliant Ana Gabriela Vasconcelos, we're excited to present a preprint ["Differential Expression Analysis for Spatially Correlated Data"](https://www.biorxiv.org/content/10.1101/2024.08.02.606405v1.full){target="_blank"} [@Vasconcelos2024], discussing exciting opportunities and unique challenges fundamental to differential expression analyses with spatial data.

We've implemented a number of tools for methods discussed in the paper in the R package [smiDE](https://github.com/Nanostring-Biostats/smiDE){target="_blank"}.  Vignettes and examples for using the techniques described below are available on the package's github page.

## Opportunities

While Differential Expression (DE) analysis has long been a core component of gene expression studies, spatially resolved transcriptomic (SRT) data presents opportunities for addressing fundamental questions regarding how cells respond to their local environment and interact with other cell types.
...
*  How does the expression of genes in T cells differ when located in stroma vs. tumor bed?
*  How does expression of genes in cancer cells in the tumor bed differ based on distance to the nearest T-cell?
*  How is gene expression for T-cells in stroma modulated by distance to the tumor bed?
...
While the types of exciting questions we can tackle with DE are limitless, there are unique challenges associated with SRT technologies that can lead to false discoveries when not properly addressed.
...
The picture below shows KRT17 transcripts from neighboring epithelial cells falsely assigned to the macrophage cell inside a tumor region.
...
Even if the number of falsely assigned transcripts is small, these errors are correlated with our DE question in a way that confounds the analysis.  Namely, the false positive KRT17 transcripts are more likely than not to occur in tumor regions compared to elsewhere because they come from the tumor-native epithelial cells!  

We could describe this dilemma for a DE analysis through a directed-acyclic graph, where the neighboring KRT17 expression in epithelial cells acts as a confounder.
...
Below is a side-by-side summary of volcano plots before and after implementing these approaches, where genes are colored by their plausible cell type annotation from the Human Protein Atlas, and marked by whether or not the gene passes the contamination metric. 
...
The ability to visually assess the behavior of the random effects can provide further context for the analysis.  For example, CD79A retains the significance in the SRE model, showing several concentrated regions of high expression captured by the random effect.  For WNT7A, numerous small regions of autocorrelated expression reduce the significance of the gene when taken into context of the model.
...
Here we've discussed exciting opportunities for DE analysis with SRT datasets, along with unique challenges this data type poses.
All of the analysis tools described here are implemented in the [smiDE R package](https://github.com/Nanostring-Biostats/smiDE){target="_blank"}, with syntax examples and vignettes which can be found on the github page.  We hope the reader will also check out the [preprint](https://www.biorxiv.org/content/10.1101/2024.08.02.606405v1.full){target="_blank"} for more thorough detail on the our study of DE with spatially correlated data.

# Resources

- [preprint](https://www.biorxiv.org/content/10.1101/2024.08.02.606405v1.full){target="_blank"}
- [smiDE R package](https://github.com/Nanostring-Biostats/smiDE){target="_blank"}
