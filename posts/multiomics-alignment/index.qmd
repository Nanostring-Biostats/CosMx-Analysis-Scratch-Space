...
:::{.callout-note}
## Update for AtoMx<sup>&#174;</sup> SIP 2.2+
If working with multiomics data acquired with AtoMx SIP 2.2+, the protein and RNA datasets are now aligned during acquisition which streamlines the process
of aligning RNA transcript to protein segmentation output to a single button (@fig-fig7).
:::
...
Download and unzip the `MultiO_Alignment_Tool_V1.4.1.zip` file available [here](https://github.com/Nanostring-Biostats/CosMx-Analysis-Scratch-Space/releases/tag/multiomics){target='_blank'}. Once the file has been unzipped, launch the tool by double-clicking the `MultiOAligmentUI.bat` file. The executable will launch an interactive graphical user interface (GUI) as shown in @fig-gui.
...
# Starting with AtoMx SIP 2.2

To align RNA transcripts to protein segmentation for AtoMx SIP 2.2+, simply point
the `MultiO_Alignment_Tool v2` to your segmentation folder and transcripts folder
and press the `Only reassign transcripts` button on the lower left of the application 
(@fig-fig7).


```{r}
#| label: fig-fig7
#| fig-cap: "Screenshot of new version of the `MultiO_Alignment_Tool` with additional button and required inputs."
#| out-width: "100%"
#| echo: false
