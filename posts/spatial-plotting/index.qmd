...
[Plotting and analysis with Squidpy](../squidpy-essentials/index.qmd)
...
Overlay polygons with metadata such as cell type. Here we're using Leiden clustering results. There are a small number of cells with no associated cell type as they got flagged in QC. These show up as `NA` in the plots.
...
plot.df$alpha <- ifelse(plot.df$cell_type == "5", 1, 0.3)
...
ggplot(plot.df,
       aes(x = x_global_px, y = y_global_px, group = cell,
           fill = as.factor(cell_type), alpha = alpha)) +
  geom_polygon(color = "darkgrey") +
  coord_fixed() +
  theme_void() +
  labs(fill = "Cell type") +
...
We start by reading in the outputs of this module for use in multiple of the plots in this section.
