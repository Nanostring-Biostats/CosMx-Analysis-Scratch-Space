...
- @sec-working-with-layers shows basic GUI tips for interacting with SMI data
...
these pre-processing steps as they are not required.
...
There are a few ways to rearrange. First, we could manually move folders.
...
Second, we can create a new folder in terminal and then use symbolic links to 
map the example data in the correct format. Here's an example of doing that in linux/mac.
...
Note that the Seurat file contains _both_ tissues of mouse brain dataaset. We need to filter
...
to do: a picture of what the "final" product looks like would be helpful.
