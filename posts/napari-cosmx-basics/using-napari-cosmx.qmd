...
description: napari-cosmx is flexible enough to quickly explore SMI data in a GUI
  ...
  ways in which we can use napari-cosmx to view SMI data. I will make use of this
...
User Interface yet robust enough to script reproducible results and tap into
...
FFPE that is available to downloaded from NanoString's website [here](https://nanostring.com/products/cosmx-spatial-molecular-imager/ffpe-dataset/cosmx-smi-mouse-brain-ffpe-dataset/){target="blank"}. We'll need to perform some slight file structure changes
...
following metadata columns so we can view them: 

- RNA_nbclust_clusters
- RNA_nbclust_clusters_long
- spatialClusteringAssignments

Note that the Seurat file contains _both_ public datasets. We need to filter
...
layers visible (@fig-initial; Initial View tab). These include `FOV labels` and `Segmentation`. Clicking the eye
...
(@fig-cell-types). In the `Color Cells` widget, we can control which cell types
we would like to view. We can also see the cell type (and other metadata) for 
individual cells but making the making visible the `Metadata` layer, clicking 
on the layer, and hovering over cells of interest. When hovering, the cell 
metadata will be displayed on the bottom of napari. To view the IF channels, use
the `Morphology Images` widget, In @fig-IF I turned off the visibility of the cell
...
@fig-tx, I zoomed in on a section of the cortex and plotted _Calb1_.
...
<!-- ```{r} -->
<!-- #| eval: false -->
<!-- Idents(sem) <- sem@meta.data$RNA_nbclust_clusters_long -->
<!-- markers1 <- FindMarkers(object = sem, ident.1 = "Excitatory_neurons_Layer1_Piriform", assay="RNA", slot="data") -->
<!-- markers1['Calb1',] -->
<!-- markers23 <- FindMarkers(object = sem, ident.1 = "Excitatory_neurons_Layer2_3", assay="RNA", slot="data") -->
<!-- markers23['Crhr1',] -->
<!-- row.names(head(markers23 %>% arrange(p_val, -abs(avg_log2FC)))) -->


<!-- markers4 <- FindMarkers(object = sem, ident.1 = "Excitatory_neurons_Layer4", assay="RNA", slot="data") -->
<!-- markers56 <- FindMarkers(object = sem, ident.1 = "Excitatory_neurons_Layer5_6", assay="RNA", slot="data") -->
<!-- markers6 <- FindMarkers(object = sem, ident.1 = "Excitatory_neurons_Layer6", assay="RNA", slot="data") -->
...
that can found loaded in the `>_` ipython interpeter. In fact, here's a script
that can help reproduce the figures above.
...
gem.viewer.camera.zoom = 125.266673507691
...
cell_type_layer.opacity = 0.5
cell_type_layer.visible = True
gem.viewer.camera.center = (0.0, -0.09967980655740494, -58.955433742224685)
gem.viewer.camera.zoom = 578.8381123472684
gem.viewer.layers['Segmentation'].visible = True
gem.viewer.layers['Segmentation'].opacity = 0.2
gem.plot_transcripts(gene = "Calb1", color = 'magenta', point_size=20) # I
...
'jot down' the results. For example, when I zoom or pan to another location, 
...
We can plot similar genes or targets with the same color:

```{r}
#| eval: false
to do: gem targets that are negative then loop
negatives = 
[gem.plot_transcripts(gene = x, color = "white", point_size=10) for x in negatives];

```

And we can combine this with a list of tuples:

```{r}
#| eval: false
genes = [('to do', "orange"), ("", "cyan"), ("", "yellow")]
...
Directly specifying the cell type colors is possible. 
