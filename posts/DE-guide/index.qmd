...
The patchDE algorithm is available as a .R script [here](https://github.com/Nanostring-Biostats/CosMx-Analysis-Scratch-Space/blob/Main/_code/PatchDE/DEutils.R){target="_blank"}.
...
**Warning**: most meta-analyses, including the version demonstrated below, assume each patch is independent. 
Under the realistic scenario where neighboring patches will have correlated results, this assumption fails, producing over-optimistic p-values. 
...
topgenes1 <- setdiff(names(lowestp)[order(lowestp, decreasing = TRUE)[1:50]], NA)
...
pheatmap::pheatmap((deresults$dist2tcell$ests * (deresults$dist2tcell$pvals < 1e-3))[topgenes1, ],
