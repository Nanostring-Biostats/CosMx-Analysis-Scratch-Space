...
dist2tcell <- FNN::get.knnx(
  ...
  query = as.matrix(xy[inds.tumor, , drop=FALSE]),
  k = 1
)$nn.dist[, 1]
...
res <- varrymander(xy = xy[inds.tumor, ], 
                   X = dist2tcell, 
                   dbscan_eps = 0.05, maxdbsize = 50, 
                   maxradius = 0.25,
                   totvarthresh = .25  # recommend default of 100 for most cases 
)
str(res)

# to view, draw polygon boundaries around each patch:
polys <- list()
for (patch in setdiff(unique(res), NA)) {
  inds <- (res == patch) & !is.na(res)
  polys[[as.character(patch)]] <- alphahull::ahull(xy[inds.tumor, ][inds, ], alpha = 0.5)
}
plot(xy[inds.tumor, ], pch = 16, cex = 0.3, asp = 1,
     main = "Tumor cells colored by distance to T-cell",
     col = colorRampPalette(c(c("#A6CEE3","#1F78B4","#081D58", "black")))(101)[
...
for (i in 1:length(polys)) {
  plot(polys[[i]], add = TRUE, wpoints = F, col = "black")
}
```

```{r echo = FALSE, eval = TRUE}
knitr::include_graphics("figures/varrymander.png")
```
...
deresults <- list()
for (bin in setdiff(unique(res), NA)) {
  thisbininds <- (res == bin) & !is.na(res)
  deresults[[bin]] <- hastyDE(
    y = norm[inds.tumor, ][thisbininds, ],
    df = data.frame("dist2tcell" = dist2tcell[thisbininds])
  )
}

### explore the results:
pvals <- sapply(deresults, function(tmp){tmp$p})
ests <- sapply(deresults, function(tmp){tmp$effect})
ses <- sapply(deresults, function(tmp){tmp$se})
rownames(pvals) <- rownames(ests) <- rownames(ses) <- colnames(norm)

# find genes with strong results in any region
lowestp <- apply(pvals, 1, min)
topgenes1 <- names(lowestp)[order(lowestp, decreasing = TRUE)[1:50]]

# use meta-analysis to summarize genes' trends across patches
metapvals <- sapply(rownames(pvals), function(gene) {
  fit <- metafor::rma.uni(yi = ests[gene, ], sei = ses[gene, ], method = "REML")
  return(fit$pval)
})
