...
xy <- meta_data[, c("x_slide_mm", "y_slide_mm")]  # note we're transposing the axes to make the plots fit better in this doc
...
```{r echo = FALSE, eval = TRUE}
knitr::include_graphics("figures/dist2vessel.png")
```
...
which accompanies our differential expression package [smiDE](https://nanostring-biostats.github.io/CosMx-Analysis-Scratch-Space/posts/smiDE/){target="_blank"}.
...
2. **In the remaining genes, adjust for minor bias.** We measure a gene's potential to suffer 
 misassignment into a cell using its mean expression around the cell. This then becomes a term in
 DE models. Taking this step mitigates, though does not fully remove, bias from segmentation errors. 
...
Poisson models offer a middle ground. 
...
## Approaches for maximal speed

### Simultaneous fitting of all genes using linear algebra
...
out <- hastyDE(y = norm[inds.tumor, ], 
               df = data.frame("dist2tcell" = dist2tcell)) 
str(out)
```

### Using subsets to reduce spatial correlation for hasty DE

We can optionally take a modestly effective if statistically wasteful countermeasure to spatial correlation: 
 rather than including all cells of our cell type, we use a subset of cells sampled to ensure some reasonable spatial distance between all cells. 
In other words, we can weaken the effect of spatial correlation by tanking our sample size, and therefore our statistical power. In studies involving large numbers of cells over a large tissue, this can be effective.
E.g., in a whole slide study of ~400 FOVs, we can sample just 12 cells per FOV, leaving a sizeable spatial buffer between cells while still keeping 4800 cells for analysis, an ample number to detect genes with reasonable effect sizes.
Because of this approach's statistical wastefulness, we recommend it for exploratory analysis only,
 not for publication. 
...
4. Summarize each gene's total evidence with a meta-analysis. Wew have had luck with the metafor package. 
...
This lets us run the super-fast \code{hastyDE} algorithm above within each path. 
...
The varrymander algorithm is available as a .R script [here](___________){target="_blank"}.
Below is a demonstration of its use:


```{r echo = TRUE, eval = FALSE}
### use varrymander to partition the tissue into patches:
res <- varrymander()
...
In other words, the spatial determinants of gene expression are far more complicated than the DE models 
 our limited biological knowledge is able to produce. 
