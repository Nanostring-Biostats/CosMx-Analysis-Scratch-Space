---
title: "Unlocking Spatial Insights: Multi-Omics & 3D RNA Analysis with `napari-CosMx v0.5.0`"
author:
  - name: Lidan Wu
...
description: "Supercharge your CosMx data analysis with the latest `napari-CosMx` release `v0.5.0`, now featuring seamless multi-omics integration and true 3D RNA workflows. Move from raw data to spatial discoveries faster, with interactive visualization and streamlined scripting for reproducible results."
resources:
  - figures/*.webm
draft: false
execute:
...
.feat-table table th:first-child,
.feat-table table td:first-child {
  width: 22ch;
  white-space: normal;
}
</style>
...
1. Download the latest `whl` file for `napari-CosMx v0.5.0` from [assets/napari-cosmx releases](https://github.com/Nanostring-Biostats/CosMx-Analysis-Scratch-Space/tree/Main/assets/napari-cosmx%20releases){target="_blank"}.
...
pip install napari==0.6.6
pip install C:\path\to\napari_cosmx-<version>-py3-none-any.whl
```
...
1. Export raw data for your study from AtoMx SIP (see (earlier post)[https://nanostring-biostats.github.io/CosMx-Analysis-Scratch-Space/posts/napari-cosmx-intro/#sec-export-raw-data]{target="_blank"}) and organize them by slide. Each slide folder (@fig-raw-inputs) should contain 
   ...
   - `plex*.csv`: required if it's protein data; csv file listing the protein names (`ProbID` & `DisplayName`) included in your protein data. 
...
#| fig-cap: "Raw data inputs for `Stitch Images (napari CosMx)` plugin."
...
knitr::include_graphics("./figures/fig-stitch-outocmes.png")
```
...
When a gene and protein share the same name (e.g. **B**), `napari-CosMx` automatically assigns distinct layer names—**B [rna]** and **B [protein]**, so you can load both and compare their spatial distributions side-by-side without confusion.
