...
1. Load in flat files exported by AtoMx<sup>&#174;</sup> SIP
2. Create a custom spatial layout of our tissues for better visualization (optional)
...
6. Perform immune cell typing using the HieraType algorithm
7. Thoroughly QC our cell typing results
...
# recommendations (on the permissive side): 20 counts for 1000plex, 50 for 6000plex, 100 for WTX
count_threshold <- 100  
...
Naming clusters used to be time-consuming, but we have found that the right prompts to an large language model (LLM), **combined with rigorous oversight**, minimize the burden of cluster naming.
...
Use \code{smiDE::overlap_ratio_metric} to do so. (smiDE is available on the scratch space.)
...
**Note: for tumor studies, additional techniques can help classify cancer cells:**
...
3. For WTX studies, it's possible to estimate cells' copy number variants. We have used inferCNV successfully, though we needed to run it on subsets of ~10k cells for it to compute. A newer method, [ClonalScope](https://github.com/seasoncloud/Clonalscope){target="_blank"}, looks promising.
