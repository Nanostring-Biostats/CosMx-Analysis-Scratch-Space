...
categories:
  ...
  - overview
  - how-tos
image: figures/DAPI-red.png
description: Best practices for CosMx RNA analysis. Post 3 of 3, covering strategies for differential expression analysis. Covers both how to define useful variables for DE and strategies for running DE.
draft: false
---
  
# Introduction

Here we provide workflows for differential expression testing. 
Also see the companion posts on [pre-processing](https://nanostring-biostats.github.io/CosMx-Analysis-Scratch-Space/posts/vignette-basic-analysis-updated/01_preprocessing.html) and [exploratory analysis](https://nanostring-biostats.github.io/CosMx-Analysis-Scratch-Space/posts/vignette-basic-analysis-updated/02_exploratory_analysis.html).
...
| **Step** | **Inputs** | **Outputs** |
|------|--------|---------|
| Defining predictor variables | xy, cell type, custom inputs | Predictor variables |
| Find safe genes via overlap ratio metric | Raw counts, xy, cell type | Genes to analyze per cell type |
| pre-DE calculations | xy | preDE (object for modeling segmentation errors) |
| “Standard” DE with smiDE | Predictor variables, Raw counts, preDE, cell type, list of genes to analyze per cell type | DE outputs: p-values, effect sizes, etc. |
| “Optimal / slow” DE with smiDE | Predictor variables, Raw counts, preDE, cell type, genes to analyze per cell type | DE outputs: p-values, effect sizes, etc. |
| “Hasty / exploratory” DE with hastyDE | Predictor variables, Normalized counts, cell type, genes to analyze per cell type | DE outputs: p-values, effect sizes, etc. |
| “Stepwise” DE with hastyDE & smiDE | Predictor variables, Normalized counts, Raw counts, cell type, genes to analyze per cell type | DE outputs: p-values, effect sizes, etc. |
| Analysis of DE results | DE results | plots and tables |
...
For continuous variables like distance to a cell type or local abundance of a cell type, it
...
For categorical variables, we can use polygons over space to denote predictor category, 
while using color to show expression:

```{r echo = FALSE, eval = TRUE, out.width = "85%"}
knitr::include_graphics("figures/depolygons.png")
```
...
knitr::include_graphics("figures/pdcd1.png")
```
 
Or we can use a heatmap to show the mean expression per category of many genes at once:

```{r echo = FALSE, eval = TRUE, out.width = "15%"}
knitr::include_graphics("figures/deheatmap2.png")
```
