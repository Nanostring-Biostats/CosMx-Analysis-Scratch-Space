...
## Fast, spatial-free screening with spatial validation
...
2. For the remaining genes, generate p-values from the NULL distribution, i.e. uniformly between 0-1. 
...
# run fast DE:
fastres <- hastyDE(y = ____, df = ____)
...
# Run smiDE on the top genes with :
...
When your models have finished, it remains to prioritize the most promising genes for reporting and further analysis, 
 then validate and explore their results.

 
**FROM DAN:** 
*In some cases, expression amongst spatially neighboring cells may be correlated due to some spatially smooth factor unrelated to our covariate of interest in DE.  This can lead to biased effect estimates, and potentially inflated statistical significance.
DE might benefit from inclusion of a spatial random effect when the magnitude of variation in the spatial random effect is small, or when the correlation between the random effect and the covariate is small.  In addition, it’s helpful for the DE covariate of interest to be more spatially nuanced than the smooth random effect we might model.   Together , these situations are settings in which  inference can benefit from including the spatial random effect.  
Given additional computational considerations, we might consider using a spatially correlated random effect when
DE question satisfies the criteria above AND
Modeling < 5,000 cells and/or
Modeling a small number of particularly interesting genes where we think the random effect appropriate.*
